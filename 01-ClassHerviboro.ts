import { Mamifero } from "./00-ClassMamifero";

export class Herviboro extends Mamifero{
  private comidaFavorita:string

  public constructor(pNombre: string, pPatas: number, pEnergia:number, pComidaFavorita:string){
    super(pNombre, pPatas, pEnergia)
    this.comidaFavorita=pComidaFavorita
  }

  public Comer(): void {
    this.energia += 10;
    console.log(`Soy ${this.nombre} y soy hervivoro, y me gusta los vegetales!. No puedo ver sufrir a los otros animales. Poreso no podria matar para comer. Mi alimento preferido es: ${this.comidaFavorita} y mi energia subio a ${this.energia}` )
  }
}